...
var createOrUpdateEvent = function(payload, event, sender, github) {
  return new Promise(function(resolve, reject) {
    ...
    var commit = {
      filename: config.paths.events + event.id + '.md',
      name: event.name
    };

    github.repos.getContent({
      user: config.github.repos.gitevent.split('/')[0],
      repo: config.github.repos.gitevent.split('/')[1],
      ...
      if (error && error.code === 404) {
        ...
        github.repos.createFile({
          ...
          //   user: config.github.repos.speakers.split('/')[0],
          //   repo: config.github.repos.speakers.split('/')[1],
          //   title: event.name,
          //   due_on: event.startDate,
          //   description: event.id
          // }, function(error) {
          //   debug('GitHub:createMilestone()');
          //
          //   if (error) {
          //     return reject(new Error(error));
          //   }
        ...
        var body = previousEvent.__content;
        delete previousEvent.__content;
...
var addTalk = function addTalk(payload, speaker, github) {
  return new Promise(function(resolve, reject) {
    ...
    var commit = {
      filename: config.paths.events + eventId + '.md'
    };
    ...
    github.repos.getContent({
      ...
      var talks = [];
      var contents = new Buffer(eventsFile.content, eventsFile.encoding).toString('utf8');
      var event = yamlFront.loadFront(contents);
      var body = event.__content;
      delete event.__content;
