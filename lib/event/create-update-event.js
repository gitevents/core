...
module.exports = function createUpdateEvent(event, github, config) {
  return new Promise(function(resolve, reject) {
    ...
    githubEventFile(github, config).get(commit.filename)
      .then(function(eventsFile) {
        ...
        githubEventFile(github, config).update(commit.filename, eventsFile.sha, file, message)
          .then(function() {
            resolve(event);
          })
      ...
      .catch(function(error) {
        if (error && error.code === 404) {
          ...
          githubEventFile(github, config).create(commit.filename, file, message)
            .then(function() {
