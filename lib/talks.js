...
var createTalk = function createTalk(commit, markdown, github) {
  return new Promise(function(resolve, reject) {
    ...
    var file = new Buffer(markdown).toString('base64');
  ...
  return new Promise(function(resolve, reject) {
    ...
    github.user.getFrom({
      ...
      parser(issue.body, function(error, body) {
        ...
        talk.id = moment(superEvent.startDate).format('YYYYMMDD') + '-' + S(issue.title).slugify().s;
        talk.performer = {
          'context': 'http://schema.org',
          'type': 'Person',
          githubId: speaker.id,
          id: S(speaker.name).slugify().s,
          name: speaker.name,
        ...
        github.repos.getContent({
          ...
          if (error && error.code === 404) {
            createTalk(commit, markdown, github).then(function() {
              resolve();
            }).catch(function(error) {
              reject(error);
            });
          } else {
            updateTalk(commit, markdown, github, talkFile.sha).then(function() {
              resolve();
            }).catch(function(error) {
              reject(error);
            });
