...
var updateEvent = function(payload, talk, github, eventsFile) {
  return new Promise(function(resolve, reject) {
    // small function to find existing events
    ...
    try {
      readableEvents = JSON.parse(new Buffer(eventsFile.content, 'base64').toString('ascii'));
    } catch (error) {
    ...
    if (foundEventId !== -1) {
      debug('Found event with event id: ' + githubEventId);

      var talkId = readableEvents[foundEventId].talks.findById(payload.issue.id);

      if (talkId !== -1) {
        debug('Updating existing talk with id: ' + payload.issue.id);
        var updatedTalk = talk;

        updatedTalk.updated_at = new Date().toJSON();
        updatedTalk.speaker = readableEvents[foundEventId].talks[talkId].speaker;
        readableEvents[foundEventId].talks[talkId] = updatedTalk;
        message = 'Updated talk by ' + updatedTalk.speaker.github;
      } else {
      ...
      github.repos.updateFile({
        ...
        resolve(readableEvents[foundEventId]);
      });
    } else {
      debug('No event found. Creating and adding talk.');

      createEvent(payload, talk, github, false).then(function(onRejected, onFulfilled) {
        debug('Added event to events.json.');
        resolve();
      });
    }
  });
...
var createEvent = function(payload, talk, github, create) {
  return new Promise(function(resolve, reject) {
        ...
        parser(payload.issue.body, function(error, body) {
          ...
          github.repos.getContent({
            ...
            if (error && error.code === 404) {
              createEvent(payload, talk, github, true).then(function(onRejected, onFulfilled) {
                resolve();
              });
            } else {
              updateEvent(payload, talk, github, eventsFile).then(function(onRejected, onFulfilled) {
                resolve();
              });
            }
          });
